...
pub fn evaluate_step(state: EvalState) -> EvaluateResult {
    ...
    match expr_content {
        ...
        SExprContent::Atom(Value::Number(n)) => {
            (frame.continuation.func)(RuntimeValue::Number(*n))
        },
        SExprContent::Atom(Value::String(s)) => {
            (frame.continuation.func)(RuntimeValue::String(s.clone()))
        },
        SExprContent::Atom(Value::Boolean(b)) => {
            (frame.continuation.func)(RuntimeValue::Boolean(*b))
        },
        SExprContent::Atom(Value::Character(c)) => {
            (frame.continuation.func)(RuntimeValue::Character(*c))
        },
        
        // 符号（变量引用）
        SExprContent::Atom(Value::Symbol(name)) => {
            match lookup_variable(name, &frame.env) {
                Some(value) => (frame.continuation.func)(value.clone()),
        ...
        // 空列表
        SExprContent::Nil => {
            (frame.continuation.func)(RuntimeValue::Nil)
        },
        
        // 其他情况
        _ => EvaluateResult::Error(EvaluateError::InvalidExpression {
            span: expr_span.clone(),
            message: format!("Unsupported expression type: {:?}", expr_content),
        }),
    }
}
